# app.py
# Version 1.5.2: Prioritizing Spreadsheet ID for Google Sheets connection.
...
default_session_state = {
    ...
    'sheet_connection_attempted_this_session': False # To attempt connection only once per session initially
}
...
# --- Google Sheets Setup ---
# Attempt connection once per session if config for sheets is present
if not st.session_state.sheet_connection_attempted_this_session and \
   (cfg.gsheets.service_account_info and (cfg.gsheets.spreadsheet_id or cfg.gsheets.spreadsheet_name)):
    ...
    st.session_state.sheet_connection_attempted_this_session = True # Mark as attempted
    st.session_state.sheet_writing_enabled = True # Assume true, will be set to false on failure

    st.sidebar.info("Attempting to connect to Google Sheets...") # User feedback
    st.session_state.gs_worksheet = data_storage.get_gspread_worksheet(
        cfg.gsheets.service_account_info,
        cfg.gsheets.spreadsheet_id,     # Pass spreadsheet_id (prioritized by data_storage)
        cfg.gsheets.spreadsheet_name,   # Pass spreadsheet_name as fallback
    ...
    if st.session_state.gs_worksheet:
        ...
        st.sidebar.error("Google Sheets connection failed. Check logs/secrets.")
elif not (cfg.gsheets.service_account_info and (cfg.gsheets.spreadsheet_id or cfg.gsheets.spreadsheet_name)):
    st.session_state.sheet_writing_enabled = False # No config for sheets
...
with st.sidebar:
    ...
    if not st.session_state.sheet_writing_enabled:
        st.sidebar.warning("⚠️ Google Sheets integration disabled or connection failed.")
...
if start_button_val:
    ...
    if st.session_state.results_data and st.session_state.sheet_writing_enabled and st.session_state.gs_worksheet:
        ...
        rows_written = data_storage.write_data_to_sheet(st.session_state.gs_worksheet, st.session_state.results_data, extraction_query_text=extraction_query_for_sheet)
    ...
    if st.session_state.results_data and llm_key_available and (enable_llm_summary_val or llm_extract_query_input_val.strip()):
        ...
        with st.spinner("Generating consolidated overview..."):
            ...
            if not all_individual_summaries and llm_extract_query_input_val.strip():
            ...
            if not all_individual_summaries:
                st.warning("No valid LLM outputs to consolidate."); st.session_state.consolidated_summary = "Error: No valid LLM outputs for consolidation."; st.session_state.processing_log.append("  ❌ No valid LLM outputs for consolidation.")
            else:
                llm_api_key_to_use = cfg.llm.google_gemini_api_key if cfg.llm.provider == "google" else cfg.llm.openai_api_key
                llm_model_to_use = cfg.llm.google_gemini_model if cfg.llm.provider == "google" else cfg.llm.openai_model_summarize
                consolidated_summary_text = llm_processor.generate_consolidated_summary(all_individual_summaries, topic_context=topic_for_consolidation, api_key=llm_api_key_to_use, model_name=llm_model_to_use, max_input_chars=cfg.llm.max_input_chars)
...
# --- Footer for Version ---
st.markdown("---")
st.caption("Keyword Search & Analysis Tool v1.5.2")
