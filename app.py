# app.py
# Version 1.4.1: Fixed NameError for keywords_list_val in consolidation UI.
...
# --- Main Area for Results & Log - DEFINE CONTAINERS EARLY ---
...
if start_button_val:
    ...
    st.session_state.last_keywords = keywords_input_val # Save current input
    st.session_state.last_extract_query = llm_extract_query_input_val
    
    # This keywords_list_val is local to this "if start_button_val:" block
    ...
    progress_bar_placeholder = st.empty() # Use a placeholder for the progress bar
                ...
                if is_good_scrape:
                    ...
                    if llm_key_available:
                        ...
                        if enable_llm_summary_val:
                            ...
                            item_data_val["llm_summary"] = summary; st.session_state.processing_log.append(f"        Summary: {summary[:100] if summary else 'Failed/Empty'}..."); time.sleep(0.1)
                        if llm_extract_query_input_val.strip():
                            ...
                            item_data_val["llm_extracted_info"] = extracted_info; st.session_state.processing_log.append(f"        Extracted: {extracted_info[:100] if extracted_info else 'Failed/Empty'}..."); time.sleep(0.1)
...
# This block is now evaluated on every rerun if conditions are met
if st.session_state.results_data and llm_key_available:
    st.markdown("---")
    st.subheader("✨ Consolidated Overview")

    # *** MODIFICATION HERE ***
    # Use st.session_state.last_keywords to get the keywords list
    # This ensures it's available even if start_button_val is not True in the current rerun
    if st.session_state.last_keywords:
        current_keywords_for_consolidation = [
            k.strip() for k in st.session_state.last_keywords.replace(',', '\n').split('\n') if k.strip()
        ]
    else: # Fallback if last_keywords is somehow empty but results exist
        current_keywords_for_consolidation = []

    if not current_keywords_for_consolidation:
        topic_for_consolidation = "the searched topics"
    elif len(current_keywords_for_consolidation) == 1:
        topic_for_consolidation = current_keywords_for_consolidation[0]
    else:
        topic_for_consolidation = f"topics: {', '.join(current_keywords_for_consolidation[:3])}{'...' if len(current_keywords_for_consolidation) > 3 else ''}"
    ...
    if st.button(f"Generate Consolidated Overview for '{topic_for_consolidation}'", key="consolidate_button", use_container_width=True):
        with st.spinner("Generating consolidated overview... This may take a moment."):
            ...
            if not all_individual_summaries:
                st.warning("No valid individual summaries available to consolidate.")
                st.session_state.consolidated_summary = "Error: No valid individual summaries for consolidation."
            else:
...
if st.session_state.get('consolidated_summary'):
    # Ensure this display section uses a container defined at the top level
    with results_container:
        st.markdown("---") # Separator before consolidated summary if individual results are also shown
...
# --- Footer for Version ---
st.markdown("---")
st.caption("Keyword Search & Analysis Tool v1.4.1")
