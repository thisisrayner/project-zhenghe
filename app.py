# app.py
# Version 1.5.1: Fixed SyntaxError in LLM insights display block.
...
if cfg.gsheets.service_account_info and cfg.gsheets.spreadsheet_name:
    ...
    if st.session_state.gs_worksheet is None:
        st.session_state.gs_worksheet = data_storage.get_gspread_worksheet(
            cfg.gsheets.service_account_info, cfg.gsheets.spreadsheet_name, cfg.gsheets.worksheet_name
        )
        if st.session_state.gs_worksheet: data_storage.ensure_header(st.session_state.gs_worksheet)
...
with st.sidebar:
    ...
    if not st.session_state.sheet_writing_enabled:
        st.sidebar.caption("⚠️ Google Sheets integration not configured or failed.")
...
# --- Footer for Version ---
st.markdown("---")
st.caption("Keyword Search & Analysis Tool v1.5.1")
