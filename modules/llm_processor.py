...
def configure_gemini(
    api_key: Optional[str], force_recheck_models: bool = False
) -> bool:
    """
    ...
    if (
        _GEMINI_CONFIGURED
        and not force_recheck_models
        and _AVAILABLE_MODELS_CACHE is not None
    ):
        return True
    if not api_key:
    ...
    try:
        ...
        if _AVAILABLE_MODELS_CACHE is None or force_recheck_models:
            ...
            try:
                ...
                if not _AVAILABLE_MODELS_CACHE:
                    try:
                        st.warning(
                            "LLM_PROCESSOR: No Gemini models found supporting 'generateContent'. LLM features disabled."
                        )
                    except Exception:
                        print(
                            "LLM_PROCESSOR_WARNING: No Gemini models found supporting 'generateContent'. LLM features disabled."
                        )
                    _GEMINI_CONFIGURED = False
                    return False
                ...
                try:
                    st.error(
                        f"LLM_PROCESSOR: Error listing Gemini models: {e_list_models}. LLM features may be impaired."
                    )
                except Exception:
                    print(
                        f"LLM_PROCESSOR_ERROR: Error listing Gemini models: {e_list_models}. LLM features may be impaired."
                    )
        ...
        try:
            st.error(
                f"LLM_PROCESSOR: Failed to configure Google Gemini client: {e_configure}. LLM features disabled."
            )
        except Exception:
            print(
                f"LLM_PROCESSOR_ERROR: Failed to configure Google Gemini client: {e_configure}. LLM features disabled."
            )
        _GEMINI_CONFIGURED = False
    ...
    if _AVAILABLE_MODELS_CACHE is not None:  # Check if cache is populated
        if model_name not in _AVAILABLE_MODELS_CACHE:
            ...
            if prefixed_attempt in _AVAILABLE_MODELS_CACHE:
                ...
                print(
                    f"LLM_PROCESSOR Warning: Model '{model_name}' not in available list. Prefixed='{prefixed_attempt}'. Available: {_AVAILABLE_MODELS_CACHE}"
                )
    ...
    while current_retry <= max_retries:
        try:
                ...
                if (
                    ...
                    if response.prompt_feedback.safety_ratings:
                        for rating in response.prompt_feedback.safety_ratings:
                            if rating.blocked:
                                reason_message += (
                                    f" Blocked by safety category: {rating.category}."
                                )
            ...
            return (
                ...
                else "LLM_PROCESSOR: Received empty text response or response has no text attribute."
            )
            ...
            is_rate_limit_error = (
                ...
                or (
                    hasattr(e, "details")
                    and callable(getattr(e, "details"))
                    and "Quota" in e.details()
                )
                or (
                    hasattr(e, "code")
                    and callable(getattr(e, "code"))
                    and e.code().value == 8
                )
            )
            if is_rate_limit_error and current_retry < max_retries:
                current_retry += 1
                print(
                    f"LLM_PROCESSOR: Rate limit hit for '{validated_model_name}'. Retrying in {current_backoff:.1f}s (Attempt {current_retry}/{max_retries}). Error: {str(e)[:150]}..."
                )
                time.sleep(current_backoff)
                current_backoff = min(
                    current_backoff * 2 + random.uniform(0, 1.0), max_backoff_seconds
                )
            else:
                print(
                    f"LLM_PROCESSOR: Unrecoverable error for '{validated_model_name}': {e.__class__.__name__} - {str(e)[:300]}"
                )
...
def generate_summary(
    ...
    truncated_text = _truncate_text_for_gemini(
...
def _parse_score_and_get_content(
    text_with_potential_score: Optional[str],
    ...
    match = re.match(
        r"^\s*Relevancy Score:\s*(\d)\s*/\s*5\s*\n?(.*)",
        text_with_potential_score,
        re.DOTALL,
    )
    ...
    if match:
        try:
            ...
            print(
                f"LLM_PROCESSOR_DEBUG (parse_score): Regex Matched! Score: {score}, Content Preview: '{content_text[:70]}...'"
            )
        except (IndexError, ValueError) as e_parse:
            score = None
            content_text = (
                text_with_potential_score  # Fallback to original if parsing group fails
            )
            print(
                f"LLM_PROCESSOR_DEBUG (parse_score): Regex matched but group parsing failed. Error: {e_parse}. Original text: '{text_with_potential_score[:100]}...'"
            )
    else:
        ...
        print(
            f"LLM_PROCESSOR_DEBUG (parse_score): Primary regex FAILED. Text: '{text_with_potential_score[:100]}...'"
        )
        if text_with_potential_score.strip().startswith("Relevancy Score:"):
            ...
            score_match_simple = re.search(
                r"Relevancy Score:\s*(\d)\s*/\s*5", first_line
            )
            if score_match_simple:
                try:
                    ...
                    if "\n" in text_with_potential_score.strip():
                        content_text = (
                            text_with_potential_score.strip().split("\n", 1)[1].strip()
                        )
                    ...
                    print(
                        f"LLM_PROCESSOR_DEBUG (parse_score): Fallback startswith + regex search matched! Score: {score}"
                    )
                except (IndexError, ValueError):
                    score = None  # Reset if fallback parsing fails
                    content_text = text_with_potential_score  # Revert to full text
                    print(
                        f"LLM_PROCESSOR_DEBUG (parse_score): Fallback parsing failed after initial match."
                    )
            # else:
...
def generate_consolidated_summary(
    ...
    is_primary_focused_consolidation_active = bool(
        extraction_query_for_consolidation
        and extraction_query_for_consolidation.strip()
    )
    is_secondary_enrichment_active = bool(
        secondary_query_for_enrichment and secondary_query_for_enrichment.strip()
    )
    ...
    for item_text in summaries:
        ...
        if any(
            lower_item_text.startswith(prefix)
            for prefix in [
                "llm error",
                "no text content",
                "llm_processor: no text content",
                "llm_processor error:",
                "llm_processor: no extraction query",
    ...
    combined_texts = "\n\n---\n\n".join(
        [
            f"Source Snippet {i+1}:\n{text_entry}"
            for i, text_entry in enumerate(texts_for_llm_input)
        ]
    )
    truncated_combined_text = _truncate_text_for_gemini(
        combined_texts, model_name, max_input_chars
    )
    ...
    if (
        llm_response
        and not llm_response.lower().startswith("llm error")
        and not llm_response.lower().startswith("llm_processor error")
    ):
        if "TLDR:" in llm_response:
            ...
            print(
                "LLM_PROCESSOR_WARNING: Consolidated summary: 'TLDR:' section title missing from LLM output."
            )
...
def generate_search_queries(
    ...
    if not configure_gemini(api_key):
        print(
            "LLM_PROCESSOR Warning: Gemini not configured for generating search queries."
        )
        return None
    ...
    if has_q1:
        context_lines.append(
            f'User\'s primary information goal (Query 1): "{specific_info_query.strip()}".'
        )
    if has_q2:
        context_lines.append(
            f'User\'s secondary information goal (Query 2): "{specific_info_query_2.strip()}".'
        )
    if not has_q1 and not has_q2:
        context_lines.append(
            "User has not provided any specific information goals beyond initial keywords."
        )
    context_for_llm = "\n".join(context_lines)
    truncated_context = _truncate_text_for_gemini(
        context_for_llm, model_name, max_input_chars
    )
    prompt = (
    ...
    response_text = _call_gemini_api(
        ...
        [prompt],
        generation_config_args={
            "max_output_tokens": num_queries_to_generate * 50,
            "temperature": 0.55,
        },
    )
    if (
        response_text
        and not response_text.startswith("LLM Error")
        and not response_text.startswith("LLM_PROCESSOR Error")
    ):
        generated_queries = [q.strip() for q in response_text.split("\n") if q.strip()]
        generated_queries = [
            re.sub(r'^(["\'])(.*)\1$', r"\2", q) for q in generated_queries
        ]  # Remove surrounding quotes
        return generated_queries[:num_queries_to_generate]
    print(
        f"LLM_PROCESSOR Warning: Failed to generate search queries or error occurred. Response: {response_text}"
    )
    return None
...
if __name__ == "__main__":
    ...
    st.title(
        "LLM Processor Module Test (Google Gemini v1.9.10 - Score Parsing Test)"
    )  # Updated title
    GEMINI_API_KEY_TEST = st.text_input(
        "Enter Gemini API Key for testing:",
        type="password",
        key="main_api_key_test_llm_v1910",
    )
    MODEL_NAME_TEST = st.text_input(
        "Gemini Model Name for testing:",
        "models/gemini-1.5-flash-latest",
        key="main_model_name_test_llm_v1910",
    )
    ...
    if configured_for_test:
        ...
        with st.expander("Test Generate Search Queries (Cached)", expanded=False):
            # ... (content from v1.9.9) ...
            test_original_keywords_str_gsq = st.text_input(
                "Original Keywords (comma-separated):",
                "home office setup, ergonomic chair",
                key="test_orig_kw_gsq_v1910",
            )
            test_specific_info_query_gsq = st.text_input(
                "Specific Info Goal (Q1 - Optional):",
                "best chair for back pain under $300",
                key="test_spec_info_q1_gsq_v1910",
            )
            test_specific_info_query_2_gsq = st.text_input(
                "Specific Info Goal (Q2 - Optional):",
                "desk height for standing",
                key="test_spec_info_q2_gsq_v1910",
            )
            test_num_queries_to_gen_gsq = st.number_input(
                "Number of New Queries to Generate:",
                min_value=1,
                max_value=10,
                value=2,
                key="test_num_q_gen_gsq_v1910",
            )
            if st.button(
                "Test Query Generation (Cached)", key="test_query_gen_button_gsq_v1910"
            ):
                test_original_keywords_tuple = tuple(
                    k.strip()
                    for k in test_original_keywords_str_gsq.split(",")
                    if k.strip()
                )
                if not test_original_keywords_tuple:
                    st.warning("Please enter original keywords.")
                else:
                    with st.spinner("Generating new search queries..."):
                        new_queries = generate_search_queries(
                            ...
                            specific_info_query=(
                                test_specific_info_query_gsq
                                if test_specific_info_query_gsq.strip()
                                else None
                            ),
                            specific_info_query_2=(
                                test_specific_info_query_2_gsq
                                if test_specific_info_query_2_gsq.strip()
                                else None
                            ),
                    ...
                    if new_queries is not None:
                        ...
                        st.write("No new queries generated or error.")

        # Test Individual Summary (Cached) - code unchanged
        with st.expander("Test Individual Summary (Cached)", expanded=False):
            # ... (content from v1.9.9) ...
            sample_text_summary_cached = st.text_area(
                "Sample Text for Summary:",
                "The Eiffel Tower is a famous landmark in Paris, France.",
                height=100,
                key="test_sample_summary_text_v1910_cached",
            )
            if st.button(
                "Test Summary Generation (Cached)",
                key="test_summary_gen_button_v1910_cached",
            ):
                with st.spinner(f"Generating summary..."):
                    summary_output = generate_summary(
                        sample_text_summary_cached,
                        GEMINI_API_KEY_TEST,
                        model_name=MODEL_NAME_TEST,
                    )
                st.markdown("**Generated Summary (Plain Text Expected):**")
                st.text(summary_output)

        # Test Specific Information Extraction (Cached) - Focus for scoring debug
        with st.expander(
            "Test Specific Information Extraction (Cached & Score Parsing)",
            expanded=True,
        ):  # Expanded for focus
            sample_text_extraction_cached = st.text_area(
                "Sample Text for Extraction:",
                "The main product is WidgetPro and it costs $50. Contact sales@example.com for more details. Another feature is its durability. It is manufactured in Germany.",
                height=100,
                key="test_sample_extraction_text_v1910_cached",
            )
            extraction_query_test_cached = st.text_input(
                "Extraction Query:",
                "product name and contact email",
                key="test_extraction_query_input_v1910_cached",
            )
            if st.button(
                "Test Extraction & Score Parsing (Cached)",
                key="test_extraction_score_button_v1910_cached",
            ):
                if not extraction_query_test_cached:
                    ...
                    with st.spinner(f"Extracting info from LLM..."):
                        llm_raw_output = extract_specific_information(
                            sample_text_extraction_cached,
                            extraction_query_test_cached,
                            GEMINI_API_KEY_TEST,
                            model_name=MODEL_NAME_TEST,
                        )
                    ...
                    if llm_raw_output:
                        parsed_score, parsed_content = _parse_score_and_get_content(
                            llm_raw_output
                        )
                        st.markdown(f"**Parsed Score:** `{parsed_score}`")
                        st.markdown("**Parsed Content (Plain Text Expected):**")
            ...
            manual_test_text_score = st.text_area(
                "Text to manually test parsing:",
                "Relevancy Score: 3/5\nThis is some content.",
                key="manual_test_score_parse_v1910",
            )
            if st.button("Manually Test Parser", key="manual_parse_button_v1910"):
                m_score, m_content = _parse_score_and_get_content(
                    manual_test_text_score
                )
                st.write(f"Manual Parse -> Score: {m_score}, Content: '{m_content}'")

        # Test Consolidation (Narrative + TLDR) - code unchanged
        with st.expander(
            "Test Consolidation (Narrative + TLDR)", expanded=False
        ):  # Collapsed by default now
            # ... (content from v1.9.9, ensure keys are unique if needed e.g. _v1910) ...
            st.write(
                "Input texts. LLM should produce plain text narrative with paragraphs separated by blank lines, then 'TLDR:' with dash-prefixed points each on a new line."
            )
            cs_text1_v1910 = st.text_area(
                "Text 1:",
                "Relevancy Score: 4/5\nSolar panel efficiency research focuses on perovskite materials. Polysilicon panels degrade at 0.5% annually.",
                height=80,
                key="cs_text1_v1910",
            )
            cs_text2_v1910 = st.text_area(
                "Text 2:",
                "Relevancy Score: 3/5\nInverters last 10-15 years. Maintenance is a cost factor.",
                height=80,
                key="cs_text2_v1910",
            )
            cs_text3_v1910 = st.text_area(
                "Text 3:",
                "Summary: Wind energy is another option with different challenges.",
                height=80,
                key="cs_text3_v1910",
            )

            cs_topic_v1910 = st.text_input(
                "General Topic Context:", "Renewable Energy Tech", key="cs_topic_v1910"
            )
            cs_q1_v1910 = st.text_input(
                "Q1 for Consolidation (Primary Focus):",
                "solar panel tech and performance",
                key="cs_q1_v1910",
            )
            cs_q2_v1910 = st.text_input(
                "Q2 for Enrichment (Secondary Context):",
                "solar system components and maintenance",
                key="cs_q2_v1910",
            )

            if st.button(
                "Test Focused Consolidation (Narrative + TLDR)",
                key="focused_cs_button_v1910",
            ):
                if not cs_q1_v1910:
                    st.warning("Please enter Q1 for primary focus.")
                else:
                    texts_cs = tuple(
                        t
                        for t in [cs_text1_v1910, cs_text2_v1910, cs_text3_v1910]
                        if t and t.strip()
                    )
                    if not texts_cs:
                        st.warning("Please provide at least one text.")
                    else:
                        with st.spinner("Generating focused summary..."):
                            output = generate_consolidated_summary(
                                summaries=texts_cs,
                                topic_context=cs_topic_v1910,
                                api_key=GEMINI_API_KEY_TEST,
                                model_name=MODEL_NAME_TEST,
                                extraction_query_for_consolidation=cs_q1_v1910.strip(),
                                secondary_query_for_enrichment=(
                                    cs_q2_v1910.strip() if cs_q2_v1910.strip() else None
                                ),
                            )
                        st.markdown(
                            "**Consolidated Output (Focused Narrative + TLDR):**"
                        )
                        st.markdown(output)

            if st.button(
                "Test General Consolidation (Narrative + TLDR)",
                key="general_cs_button_v1910",
            ):
                texts_cs = tuple(
                    t
                    for t in [cs_text1_v1910, cs_text2_v1910, cs_text3_v1910]
                ...
                if not texts_cs:
                    ...
                    with st.spinner("Generating general summary..."):
                        output = generate_consolidated_summary(
                            ...
                            secondary_query_for_enrichment=None,
                        )
                    st.markdown("**Consolidated Output (General Narrative + TLDR):**")
                    st.markdown(output)
