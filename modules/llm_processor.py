...
# Version 1.6: Refined individual summary prompts, increased default output tokens,
#              and added function for consolidated summary.
        ...
        if _AVAILABLE_MODELS_CACHE is None or force_recheck_models:
            ...
            try:
                ...
                if not _AVAILABLE_MODELS_CACHE:
                    st.warning("No Gemini models found supporting 'generateContent'.")
                    return False
            except Exception as e_list_models:
                st.error(f"Error listing Gemini models: {e_list_models}.")
                _AVAILABLE_MODELS_CACHE = []
                _GEMINI_CONFIGURED = False
                return False
        return True
    except Exception as e_configure:
        st.error(f"Failed to configure Google Gemini client: {e_configure}")
        _GEMINI_CONFIGURED = False
        return False
...
def _call_gemini_api(
    ...
    effective_generation_config = {"temperature": 0.3, "max_output_tokens": 512} # Default increased
    ...
    while current_retry <= max_retries:
        try:
            ...
            if is_rate_limit_error and current_retry < max_retries:
                current_retry += 1
                print(f"LLM Rate limit for '{validated_model_name}'. Retrying in {current_backoff:.1f}s (Attempt {current_retry}/{max_retries}). Error: {str(e)[:100]}...")
...
def generate_summary(
    ...
    prompt = (
        ...
        "Maintain a neutral and factual tone. Avoid introductory phrases like 'This text discusses...'. Go directly into the summary.\n\n"
        "--- WEB PAGE CONTENT START ---\n"
        f"{truncated_text}\n"
        "--- WEB PAGE CONTENT END ---\n\n"
        "Detailed Summary:"
    )
    return _call_gemini_api(model_name, [prompt], generation_config_args={"max_output_tokens": 512}) # Increased output tokens
...
def extract_specific_information(
    ...
    prompt = (
        ...
        "If multiple pieces of information are requested, address each one.\n\n"
        "--- WEB PAGE CONTENT START ---\n"
        f"{truncated_text}\n"
        "--- WEB PAGE CONTENT END ---\n\n"
        f"Comprehensive Extracted Information regarding '{extraction_query}':"
    )
    return _call_gemini_api(model_name, [prompt], generation_config_args={"max_output_tokens": 700}) # Increased output tokens

def generate_consolidated_summary(
    summaries: List[Optional[str]],
    topic_context: str, # e.g., the keyword or a general theme
    api_key: Optional[str],
    model_name: str = "models/gemini-1.5-flash-latest", # Consider Pro for better synthesis if available/affordable
    max_input_chars: int = 150000 # Can be larger as it's processing summaries
) -> Optional[str]:
    ...
    # Prepare a numbered list of summaries for better context for the LLM
    summary_entries = []
    for i, s in enumerate(valid_summaries):
        summary_entries.append(f"Source Summary {i+1}:\n{s}")
    combined_summaries_text = "\n\n---\n\n".join(summary_entries)

    truncated_combined_text = _truncate_text_for_gemini(combined_summaries_text, model_name, max_input_chars)
    prompt = (
        f"You are an expert analyst synthesizing information from multiple sources related to '{topic_context}'.\n"
        "The following are several summaries extracted from different web pages. Your task is to create a single, "
        ...
        f"Consolidated Overview for '{topic_context}':"
    )
    # Allow for a longer output for the consolidated summary, as it synthesizes multiple pieces.
    return _call_gemini_api(model_name, [prompt], generation_config_args={"max_output_tokens": 800})

# --- if __name__ == '__main__': block (remains similar, just update version in title) ---
if __name__ == '__main__':
    st.set_page_config(layout="wide")
    st.title("LLM Processor Module Test (Google Gemini v1.6 - Consolidation)")
    # ... (rest of the test block from v1.5, perhaps add a test for consolidation)
    ...
    sample_text_content = st.text_area("Sample Text 1 for LLM:", "The sky is blue due to Rayleigh scattering.", height=100)
    sample_text_content_2 = st.text_area("Sample Text 2 for LLM:", "Oceans appear blue because water absorbs longer wavelengths of light (reds, oranges, yellows) more strongly than blue light.", height=100)

    if configured_for_test:
        if st.button("Test Individual Summaries & Consolidation"):
            s1 = generate_summary(sample_text_content, GEMINI_API_KEY_TEST, model_name=MODEL_NAME_TEST)
            s2 = generate_summary(sample_text_content_2, GEMINI_API_KEY_TEST, model_name=MODEL_NAME_TEST)
            st.markdown("**Summary 1:**"); st.write(s1)
            st.markdown("**Summary 2:**"); st.write(s2)
            if s1 and s2:
                with st.spinner("Generating consolidated summary..."):
                    consolidated = generate_consolidated_summary([s1, s2], "Color of Sky and Ocean", GEMINI_API_KEY_TEST, model_name=MODEL_NAME_TEST)
