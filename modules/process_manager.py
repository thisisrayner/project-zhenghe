...
# Version 1.3.4:
# - Corrected placement of the final progress bar update logic to ensure
#   `current_major_step_count` and `total_major_steps_for_progress` are used
#   after the main item processing loop has completed.
# - Retains consolidated summary logic from v1.3.3 (depth for focused, plain text).
"""
...
def run_search_and_analysis(
    ...
    # Correct placement for final progress update before consolidated summary
    ...
    # --- CONSOLIDATED SUMMARY LOGIC (Unchanged from v1.3.3) ---
    if results_data and llm_key_available:
        ...
        with st.spinner("Generating consolidated overview..."):
            ...
            if texts_for_focused_summary: 
                ...
                # If primary is None but secondary contributed, could refine context further, but this is okay for now.
                ...
                if general_texts_for_consolidation:
                    ...
                    if log_message_reason: log_message_text = log_message_text.replace("." , "") + log_message_reason # Append reason if it exists
