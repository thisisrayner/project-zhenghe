...
# Version 1.3.5:
# - Updated the call to `llm_processor.generate_search_queries` to include
#   `secondary_llm_extract_query` (Main Query 2) for richer LLM-generated search queries.
# Version 1.3.4:
# - Corrected placement of the final progress bar update logic.
# - Retains consolidated summary logic from v1.3.3.
"""
...
def run_search_and_analysis(
    ...
    secondary_llm_extract_query: Optional[str] = None # This variable already exists and holds Q2
        ...
        if num_llm_terms_to_generate > 0:
            ...
            with st.spinner(f"LLM generating {num_llm_terms_to_generate} additional search queries..."):
                generated_queries: Optional[List[str]] = llm_processor.generate_search_queries(
                    ...
                    specific_info_query=primary_llm_extract_query,       # Main Query 1
                    specific_info_query_2=secondary_llm_extract_query, # Main Query 2 (NEWLY ADDED HERE)
    ...
    # --- CONSOLIDATED SUMMARY LOGIC (Remains as per v1.3.4) ---
        ...
        with st.spinner("Generating consolidated overview..."):
            ...
            if primary_llm_extract_query or secondary_llm_extract_query:
                for item in results_data:
                    ...
                    if secondary_llm_extract_query: # Check Q2 as well for focused summary input
                        ...
                        if extraction_text_q2:
                            ...
                            if score_q2 is not None and score_q2 >= 3:
                                if extraction_text_q2 not in texts_for_focused_summary: # Avoid duplicates if Q1 and Q2 are same
                                     texts_for_focused_summary.append(extraction_text_q2)
            
            # No need to sort and unique here if we only add unique above
            # texts_for_focused_summary = sorted(list(set(texts_for_focused_summary))) # Already handled by logic above
            ...
            if texts_for_focused_summary:
                ...
                if not primary_llm_extract_query and secondary_llm_extract_query: # If Q1 empty but Q2 not
                    llm_context_for_focused_summary = secondary_llm_extract_query # Use Q2 as context
                ...
                if general_texts_for_consolidation:
                    ...
                    if generated_general_overview and not str(generated_general_overview).lower().startswith("llm_processor"): # Check for prefix from llm_processor itself
                        # The generate_consolidated_summary already adds the "LLM_PROCESSOR_INFO: General overview as follows." prefix for general summaries.
                        ...
                        consolidated_summary_text_for_batch = (general_overview_info_prefix + # Keep this as a fallback if the LLM call outright fails before prefixing
