...
# Version 1.2.1:
# - Added internal _parse_score_from_extraction helper.
# - Stripped relevancy score line from Q1 extractions before adding to consolidation list.
# - Confirmed consolidated summary logic correctly focuses on Main Query 1.
"""
...
def _parse_score_from_extraction(extracted_info: Optional[str]) -> Optional[int]: # ADDED HELPER
...
def run_search_and_analysis(
    ...
    # ... (initial setup, keyword processing, LLM query gen remains same as v1.2.0) ...
    ...
    primary_llm_extract_query: Optional[str] = llm_extract_queries_input[0] if llm_extract_queries_input and llm_extract_queries_input[0].strip() else None
        ...
        if num_llm_terms_to_generate > 0:
            ...
            with st.spinner(f"LLM generating {num_llm_terms_to_generate} additional search queries..."):
                generated_queries: Optional[List[str]] = llm_processor.generate_search_queries(
                    ...
                    num_queries_to_generate=num_llm_terms_to_generate, api_key=llm_api_key_to_use_qgen, model_name=llm_model_for_query_gen
                )
            if generated_queries:
                ...
                for gq in generated_queries:
                    if gq.lower() not in current_runtime_keywords_lower:
                        keywords_list_val_runtime.append(gq); current_runtime_keywords_lower.add(gq.lower()); temp_llm_generated_set.add(gq.lower())
    ...
    if llm_key_available:
        ...
        total_llm_tasks_per_good_scrape += len([q for q in llm_extract_queries_input if q.strip()]) # Count only non-empty queries
    ...
    for keyword_val in keywords_list_val_runtime:
        ...
        for search_item_idx, search_item_val in enumerate(search_results_items_val):
            ...
            item_data_val: Dict[str, Any] = {
                "keyword_searched": keyword_val, "url": url_to_scrape_val, "search_title": search_item_val.get('title'), 
                "search_snippet": search_item_val.get('snippet'), "scraped_title": scraped_content_val.get('scraped_title'),
                "meta_description": scraped_content_val.get('meta_description'), "og_title": scraped_content_val.get('og_title'),
                "og_description": scraped_content_val.get('og_description'), "scraped_main_text": scraped_content_val.get('main_text'),
            ...
            if scraped_content_val.get('error'):
                ...
                if is_good_scrape:
                    ...
                    if llm_key_available:
                        ...
                        processing_log.append(f"        Summary: {str(summary)[:100] if summary else 'Failed/Empty'}..."); time.sleep(0.1)
                        for q_idx, extraction_query in enumerate(llm_extract_queries_input):
                            ...
                            processing_log.append(f"        Extracted (Q{q_idx+1}): {str(extracted_info)[:100] if extracted_info else 'Failed/Empty'}..."); time.sleep(0.1)
    ...
    if results_data and llm_key_available:
        ...
        with st.spinner("Generating consolidated overview..."):
            ...
            is_focused_consolidation_intended = bool(primary_llm_extract_query) # True if primary_llm_extract_query is not None and not empty
            ...
            for item in results_data:
                ...
                item_relevancy_score_q1 = _parse_score_from_extraction(extraction_text_q1) # Use helper
                ...
                if is_focused_consolidation_intended:
                    if is_extraction_q1_valid and item_relevancy_score_q1 is not None and item_relevancy_score_q1 >= 3:
                        # Strip the "Relevancy Score: X/5\n" line for cleaner input to consolidation LLM
                        parts = extraction_text_q1.split('\n', 1)
                        chosen_text_for_consolidation = parts[1] if len(parts) > 1 else parts[0]
                else: 
                    if is_summary_valid:
                        chosen_text_for_consolidation = summary_text
                    # No fallback to Q1 extraction if Q1 was empty and we're in general consolidation
            ...
            if not all_valid_llm_outputs_for_consolidation:
                ...
                if is_focused_consolidation_intended: warning_msg = f"LLM_PROCESSOR: No individual items met the minimum relevancy (from Main Query 1: '{primary_llm_extract_query}') for focused consolidated overview."
